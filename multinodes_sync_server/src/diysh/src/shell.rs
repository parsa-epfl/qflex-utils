use std::{collections::HashMap, io::{self, Write}, cmp::min, process, path::Path, fs::{self, OpenOptions}, str::FromStr, sync::mpsc::SyncSender};
...
pub struct Shell<S> {
    command_registry: HashMap<String, CommandDefinition<S>>,
...
impl<S> Shell<S> {
    pub fn new(tx: SyncSender<S>) -> Self {
        Self {  
    ...
    pub fn register_command(&mut self, definition: CommandDefinition<S>) -> &mut Self {
